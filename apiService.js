...
const apiService = {
	...
	fetchAndEnrich815Data: async (server) => {
		try {
			...
			const [
				...
				axios.get(
					`${server.url}/gateway/integracion/clientes/cuentasimple/listar?&json`,
					{
						httpsAgent: agent,
				...
				axios.get(
					`${server.url}/gateway/integracion/geografico/ciudad/listar?&json`,
					{
						httpsAgent: agent,
						headers: { Authorization: `Basic ${basicAuthToken}` },
					}
				...
				axios.get(
					`${server.url}/gateway/integracion/hardware/equipocliente/listar?json`,
					{
						httpsAgent: agent,
						headers: { Authorization: `Basic ${basicAuthToken}` },
					}
				...
				axios.get(
					`${server.url}/gateway/integracion/red/direccionip/listar?&json`,
					{
						httpsAgent: agent,
						headers: { Authorization: `Basic ${basicAuthToken}` },
					}
				...
				axios.get(
					`${server.url}/gateway/integracion/hardware/nodored/listar?activo=True&admite_clientes=True&json`,
					{
						httpsAgent: agent,
						headers: { Authorization: `Basic ${basicAuthToken}` },
					}
			...
			const mapaCiudades = {};
			responseCiudades.data.forEach(
				(item) => (mapaCiudades[item.pk] = item.fields.nombre)
			);

			const mapaEquipos = {};
			responseEquipos.data.forEach(
				(item) => (mapaEquipos[item.pk] = item.fields)
			);
			...
			return responseClientes.data.map((item) => {
				...
				cliente.equipo_cliente =
					mapaEquipos[cliente.equipo_cliente]?.nombre || "Desconocido";
				cliente.direccion_ip_815 =
					mapaIPs[cliente.direccion_ip] || "Desconocida";
				cliente.nodo_de_red_815 =
					mapaNodos[cliente.nodo_de_red]?.nombre || "Desconocida";

				return cliente;
			});
		} catch (error) {
			console.error(
				`❌ Error al obtener/enriquecer datos desde ${server.name}`,
				error.message
			);
			return [];
		}
	...
	fetchWispHubClients: async (limit = 10, offset = 0) => {
		try {
			const apiKey = process.env.WISPHUB_API_KEY;
			const apiUrl = process.env.API_URL;

			const response = await axios.get(
				`${apiUrl}/api/clientes/?limit=${limit}&offset=${offset}`,
				{
					headers: {
	...
	fetchAndCombineClientsByZone: async (zoneName, ZONE_MAPPING) => {
		try {
			console.log(
				`--- Iniciando unión de clientes para la zona: ${zoneName} ---`
			);

			const correct815Entry = ZONE_MAPPING[zoneName];
			if (!correct815Entry) {
				return {
					message: `No se encontró un servidor 815 para la zona: ${zoneName}`,
				};
			}

			// Solo clientes de ese servidor 815
			const enrichedData = await apiService.fetchAndEnrich815Data(
				correct815Entry
			);
			console.log(
				`✅ Datos obtenidos de 815 (${zoneName}): ${enrichedData.length}`
			);

			// 🔥 Pedir solo clientes de WispHub para esa zona
			const wispHubClients = await apiService.fetchWispHubClientsByZone(
				zoneName
			);
			console.log(
				`✅ Datos obtenidos de WispHub (${zoneName}): ${wispHubClients.length}`
			);

			// Crear mapa de WispHub por id_servicio
			const mapaWispHub = {};
			wispHubClients.forEach((cliente) => {
				if (cliente.id_servicio) {
					mapaWispHub[cliente.id_servicio] = cliente;
				}
			});
			...
			// Unir clientes
			const clientesUnidos = enrichedData.map((cliente815) => ({
				...cliente815,
				...(mapaWispHub[cliente815.conector] || {}),
			}));

			console.log(
				`--- Unión finalizada (${zoneName}). Total clientes unidos: ${clientesUnidos.length}`
			);
			return clientesUnidos;
		} catch (error) {
			console.error(
				`❌ Error al unir clientes de la zona ${zoneName}:`,
				error.message
			);
			return [];
		}
	...
	fetchAndCombineAllClients: async (SERVERS_815) => {
		...
		const allWispHubClients = await apiService.fetchWispHubClients();
		console.log(
			`✅ Datos totales obtenidos de WispHub: ${allWispHubClients.length}`
		);
		...
		console.log(
			`--- Mapa de WispHub creado. Cantidad de entradas: ${
				Object.keys(mapaWispHub).length
			}`
		);
		...
		console.log(
			"--- Unión finalizada. Total clientes unidos:",
			clientesUnidos.length
		);
		return clientesUnidos;
	},
	...
	fetchClientByCedula: async (cedula, ZONE_MAPPING) => {
		try {
			...
			const responseWispHub = await axios.get(
				`${apiUrl}/api/clientes/?cedula=${cedula}`,
				{
					headers: {
						Authorization: `Api-Key ${apiKey}`,
						Accept: "application/json",
			...
			if (!clientesWispHub || clientesWispHub.length === 0) {
				return {
					message: `Cliente con cédula ${cedula} no encontrado en WispHub.`,
				};
			}
			...
			const clientesProcesados = await Promise.all(
				clientesWispHub.map(async (cliente) => {
					...
					if (!correct815Entry) {
						return {
							...cliente,
							warning: `Zona ${zoneName} no encontrada en mapping`,
						};
					}
					...
					try {
						...
						console.error(
							`❌ Error consultando 815 para id_servicio=${idServicio}:`,
							error.message
						);
					}
					...
					if (clientes815.length === 0) {
						return {
							...cliente,
							warning: `Cliente con id_servicio ${idServicio} no encontrado en 815`,
						};
					}
					...
					return Promise.all(
						clientes815.map(async (foundClient815) => {
							...
							try {
								...
								console.error(
									"❌ Error al consultar diagnóstico:",
									error.message
								);
			...
			console.error(
				"❌ Error al procesar la búsqueda por cédula.",
				error.message
			);
			if (error.response) {
				console.error(
	...
	getAvailableServicesFromNode: async (
		zoneName,
		nodoPk = 1400,
		ZONE_MAPPING
	) => {
		try {
			...
			const onuDisponible =
				dhcpServicio && dhcpServicio.nombre.includes("ONU")
					? dhcpServicio
					: null;
			const ipDisponible =
				dhcpServicio && dhcpServicio.ip?.ip_disponible === "1"
					? dhcpServicio.ip
					: null;
	...
	createClientIn815: async (
		...
		try {
			...
			if (!clientData?.pk) {
				return {
					message: "Cliente creado pero no se obtuvo pk",
			...
			return {
				message: "Error al crear cliente o conexión",
				error: error.message,
			};
		}
	},

	aprovisionarClientePorSerial: async (
		zoneName,
		pkConexion,
		serialForm,
		ZONE_MAPPING,
		...
		try {
			...
			console.log(
				"Conector perfil recibido:",
				conectorPerfil,
				typeof conectorPerfil
			);

			if (!conectorPerfil) {
				return {
					message: "Debe seleccionar un conector/perfil para aprovisionar.",
				};
			}

			const conectorPerfilFinal =
				typeof conectorPerfil === "number"
					? conectorPerfil
					: Number(conectorPerfil);
			...
			console.error(
				`❌ Error al aprovisionar conexión ${pkConexion}:`,
				error.message
			);
			if (error.response) {
				console.error(
			...
			return {
				message: "Error al aprovisionar conexión",
				error: error.message,
			};
		}
	},
	...
	listAvailableOnus: async (zoneName, nodoPk = 1400, ZONE_MAPPING) => {
		try {
			...
			console.error(
				`❌ Error al listar ONUs disponibles para zona ${zoneName}:`,
				error.message
			);
			return [];
		}
	...
	enrich815Client: async (cliente815, serverUrl, basicAuthToken) => {
		try {
			...
			const [responseCiudades, responseEquipos, responseIPs, responseNodos] =
				await Promise.all([
					axios.get(
						`${serverUrl}/gateway/integracion/geografico/ciudad/listar?&json`,
						{
							httpsAgent: agent,
							headers: { Authorization: `Basic ${basicAuthToken}` },
						}
					...
					axios.get(
						`${serverUrl}/gateway/integracion/hardware/equipocliente/listar?json`,
						{
							httpsAgent: agent,
							headers: { Authorization: `Basic ${basicAuthToken}` },
						}
					...
					axios.get(
						`${serverUrl}/gateway/integracion/red/direccionip/listar?&json`,
						{
							httpsAgent: agent,
							headers: { Authorization: `Basic ${basicAuthToken}` },
						}
					...
					axios.get(
						`${serverUrl}/gateway/integracion/hardware/nodored/listar?activo=True&admite_clientes=True&json`,
						{
							httpsAgent: agent,
							headers: { Authorization: `Basic ${basicAuthToken}` },
						}
			...
			const clienteEnriquecido = {
				...
				equipo_cliente:
					mapaEquipos[cliente815.fields.equipo_cliente]?.nombre ||
					"Desconocido",
				direccion_ip_815:
					mapaIPs[cliente815.fields.direccion_ip] || "Desconocida",
				nodo_de_red_815:
					mapaNodos[cliente815.fields.nodo_de_red]?.nombre || "Desconocida",
			};
	...
	fetchWisphubCedula: async (cedula) => {
		try {
			const apiKey = process.env.WISPHUB_API_KEY;
			const apiUrl = process.env.API_URL;

			if (!cedula) {
				throw new Error("Debe enviar una cédula como parámetro");
			}
			...
			console.error("❌ Error en fetchWisphubCedula:", err.message);
			throw err; // ✅ lanzamos error para que lo maneje el controlador
		}
	},
};
